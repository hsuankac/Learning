...
// enum Street {Main, North, Elm}; // Error, can not use North again
...
// Overloading the stream insertion operator to insert the string representation of the provided 
// Grocery_Item parameter into the output stream
...
// Used for test 2
// Returns a boolen depending on whether the Grocey_Item parameter is a valid enumerator or not

bool is_valid_grovery_item(Grocery_Item grocery_item)
{
    switch(grocery_item)
    {
        case Milk:
...
void display_grocery_list(const std::vector<Grocery_Item> &grocery_list)
{
	...
	{
		std::cout << grocery_item << std::endl;  // Using the overloaded operator<<
